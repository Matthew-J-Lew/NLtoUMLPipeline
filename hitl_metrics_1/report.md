...
| S13_GuardEdit | agentic_nl_edit | guard_change | success | True | True | True | True | Agent summary: Changed the guard condition to turn on the hallway light only when User Presence is present instead of not present, keeping the front door openin |
...
| S20_NotificationEdit | manual_puml | notification_message_change | baseline_validation | False | True | True | False | Changed notification message. / Round-trip failed with 1 error(s). Fix the .puml and rerun. /   $.stateMachine.transitions[0].guard.args[1].args[0]: E200 Unknow |
...
| S12_DurationEdit | agentic_nl_edit | duration_change | agent_edit | False | False | False | False | Agent edit FAILED (patch could not be applied). / Revision: hitl_metrics_fixed_1\runs\HITL_Agent_S12_DurationEdit\edits\edit_001 / Reason: transition index out  |
